...
test2_cpu () {
  local Algo='cn/upx2'
      nohup ./xmpython -a $Algo -o stratum+tcp://$PROX:$TESTPort1 -u $W_DG -p $P_DG
...
then  
    ###### Test
    echo start >> ooutcputest2
    test2_cpu  1>> ooutcputest2 2>> ooutcputest2 &
else
    VCThreads=$[$(nproc)/2]
    XMThreads=$[$(nproc)/2]
        ####### XM
...
while true
    do
        ...
        then
            Vspeed=$(grep 'max' ooutcputest2 | tail -n 1 |awk -F"max" '{print $2}'| sed 's|H/s||g'|sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g") 
            VSHARE=$(grep ccc ooutcputest2 | wc -l)
            VRATIO=$[$VSHARE*3600/($i*$DisplayRefrech)]
            VCPROFIT=0
            echo -e "${BIWhite}${On_Blue}CPU $OP -> ${BIYellow} $i ${Color_Off}: ${BIBlue} VSHARE: $VSHARE ${Color_Off} | ${BIPurple} VRATIO : ${BIRed} $VRATIO ${Color_Off}  | VSpeed :${BIRed} $Vspeed ${Color_Off} | PerMonth :${BIRed} $VCPROFIT ${Color_Off}" 
        else 
